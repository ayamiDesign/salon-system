        ...
        try {
            DB::transaction(function () use ($handle, &$importCount) {
                ...
                while (($row = fgetcsv($handle)) !== false) {
                    ...
                    // 1つ目
                    $category1 = Category::where('name', $categoryNames[0])->first();
                    if (!$category1) {
                        $categoryMaxSortOrder++;
                        $category1 = Category::create([
                            'name' => $categoryNames[0],
                            'sort_order' => $categoryMaxSortOrder,
                        ]);
                    }

                    // 2つ目
                    $category2 = Category::where('name', $categoryNames[1])->first();
                    if (!$category2) {
                        $categoryMaxSortOrder++;
