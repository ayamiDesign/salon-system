...
<head>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const sortOrderLabels = document.querySelectorAll('.js-sort-order-label');
            ...
            saveSortButton?.addEventListener('click', function () {
                const orderedIds = Array.from(document.querySelectorAll('#pc-sortable-body .js-sort-row'))
                    .map(row => row.dataset.id);

                console.log('保存する並び順ID:', orderedIds);

                // ここで今後 fetch / ajax で保存
                // 例:
                // fetch('/categories/sort-order', { method: 'POST', ... })

                leaveSortMode(false);
                alert('並び順UIのモックです。保存処理はこれから接続します。');
            });
