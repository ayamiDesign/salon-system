...
<body class="bg-slate-50 text-slate-800">
                        ...
                        @foreach ($oldFaqs as $index => $faq)
                            <div class="faq-block rounded-2xl border border-slate-200 bg-slate-50/70 p-4 sm:p-5">
                                ...
                                <div class="mt-5 space-y-6">
                                    ...
                                    <!-- 変更メモ -->
                                    <div>
                                        <h5 class="text-sm font-semibold text-slate-900">変更メモ</h5>
                                        <p class="mt-1 text-sm text-slate-500">
                                            任意入力です。登録理由や補足があれば残してください。
                                        </p>

                                        <div class="mt-4">
                                            <textarea
                                                name="faqs[{{ $index }}][change_summary]"
                                                rows="3"
                                                placeholder="例：新店舗ルール反映のため追加"
                                                class="w-full rounded-lg border border-slate-300 bg-white px-3 py-3 text-sm leading-6 text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-accent-500 focus:ring-2 focus:ring-accent-100"
                                            >{{ $faq['change_summary'] ?? '' }}</textarea>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function addFaqBlock() {
            ...
            const index = document.querySelectorAll('.faq-block').length;
        ...
        function removeFaqBlock(button) {
            const rows = document.querySelectorAll('.faq-block');
        ...
        function updateFaqNumbers() {
            document.querySelectorAll('.faq-block').forEach((block, index) => {
                const number = index + 1;
                const numBadge = block.querySelector('.faq-number');
                const numText = block.querySelector('.faq-number-text');

                if (numBadge) numBadge.textContent = number;
                if (numText) numText.textContent = number;
            });
        }
